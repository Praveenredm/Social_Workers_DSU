...
export default function ThreeTierAc({ navigation }) {
  const [selectedSeats, setSelectedSeats] = useState([]);

  // Example seat arrangement (AC 3-Tier)
  const seats = [
    ["A1", "A2", "A3"],
    ["B1", "B2", "B3"],
    ["C1", "C2", "C3"],
  ...
  const reservedSeats = ["C1", "D2", "E3"];
  const exitSeats = ["A3", "F1"];

  const toggleSeat = (seat) => {
    if (reservedSeats.includes(seat)) return; // reserved seats cannot be selected
    if (exitSeats.includes(seat)) return; // exit row seats are fixed
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>AC 3-Tier</Text>
      </View>
